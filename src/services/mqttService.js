...
async function connectMqtt(io) {
  ...
  client.on('message', async (topic, message) => {
    try {
      ...
      const { sensorId, pH, temperature, turbidity, salinity, waterLevel } = data;

      const sensorData = {
        sensorId,
        pH: pH != null ? parseFloat(pH) : undefined,
        temperature: temperature != null ? parseFloat(temperature) : undefined,
        turbidity: turbidity != null ? parseFloat(turbidity) : undefined,
        salinity: salinity != null ? parseFloat(salinity) : undefined,
        waterLevel: waterLevel != null ? parseFloat(waterLevel) : undefined
      };
      ...
      const alerts = [];
      if (thresholds) {
        if (sensorData.pH < thresholds.pHMin || sensorData.pH > thresholds.pHMax) {
        ...
        if (sensorData.temperature < thresholds.tempMin || sensorData.temperature > thresholds.tempMax) {
          alerts.push(`Temperature out of range: ${sensorData.temperature}`);
        }
        ...
        if (sensorData.waterLevel < thresholds.waterLevelMin) {
          alerts.push(`Water Level too low: ${sensorData.waterLevel}`);
        }

        if (alerts.length > 0) {
          io.emit('alert', { sensorId, alerts, timestamp: new Date().toISOString() });
          logger.warn('Threshold alert triggered', { sensorId, alerts });
        }
      }
